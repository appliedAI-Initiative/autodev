    ...
    def create_model(self,
        ...
        :param model_path: a path to a directory containing the model/checkpoint to load or
            ...
            following:
                ...
                * a directory containing an ONNX model as generated by ONNXConverter (in which case either model.onnx
                  or model_quantized.onnx must be present)
        ...
        elif is_dir and (onnx_path.exists() or onnx_quantized_path.exists()):
            p = onnx_path if onnx_path.exists() else onnx_quantized_path
            log.info(f"Loading ONNX model from {p}")
            decoder_session, decoder_with_past_session = ORTModelForCausalLM.load_model(p,
